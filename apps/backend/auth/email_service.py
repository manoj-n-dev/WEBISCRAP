...
def _send_sync_smtp(recipient: str, subject: str, html_content: str) -> bool:
    ...
    try:
        ...
        logger.info(f"Password reset email sent successfully via SMTP to {recipient}")
        return True
...
async def send_password_reset_email(recipient: str, reset_url: str) -> dict:
    """
    Sends password reset email. If SMTP is configured, sends via SMTP server.
    Otherwise (e.g. dev/local test), logs the reset URL and returns it.
    """
    ...
    if settings.SMTP_HOST and settings.SMTP_USER:
        ...
        return {"sent": success, "method": "smtp", "reset_url": reset_url if not success else None}
    else:
